...
jid1=($(sbatch --dependency=afterok:${jid0[-1]} --account=<allocation> --partition=<queue> --time=<walltime> --gres=gpu:p100:1 --nodes=1 --ntasks-per-node=1 --mem=32G --job-name="${date}"_gpu_part --output=job_"${date}"_gpu_part.out --export=DEPENDENTJOB=${jid0[-1]} gpu_part.sh))
